function [ Fasesx,PI] = ExtraerFaseV(nom_base,tomas,mask,CP)
...
%     [m,pos]=max(propi(:,1));
    ...
    [uy,ux] = ind2sub(size(Iv),in(round(end/2)));
    PI(1,:,k) = [ux,uy];
    %figure(620), imagesc(((Iv-Io)+(Ih-Io)).*Maskx(:,:,k))
    %PI(1,:,k)=ginput(1);
    %pause();
    %figure(600+k), imagesc((Iv).*(Ih)), hold on, plot(PI(1,1,k),PI(1,2,k),'*r','MarkerSize',12)
